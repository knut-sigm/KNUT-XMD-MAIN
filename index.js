...
const config = {
  ...
  MEMORY_LIMIT: parseInt(process.env.MEMORY_LIMIT) || 2500
};
  ...
  if (fs.existsSync(cmdDir)) {
    ...
    for (const file of files) {
      try {
        ...
        // Cas 3: Obfuscation - analyser l'objet exporté
          ...
          for (const key of Object.keys(module)) {
            ...
            if (typeof potentialCmd === 'function') {
              // Essayer de deviner le nom depuis le nom de fichier
            ...
            if (potentialCmd && typeof potentialCmd === 'object') {
              ...
              for (const prop of Object.keys(potentialCmd)) {
                ...
                // Si c'est une chaîne courte, c'est probablement le nom
                if (typeof value === 'string' && value.length < 30 && !value.includes(' ') && !value.includes('http')) {
                  cmdName = value.toLowerCase();
                }
                
                // Si c'est une fonction, c'est execute
                if (typeof value === 'function') {
                  cmdExecute = value;
                }
              }
              
              // Si on a trouvé une fonction mais pas de nom, utiliser le nom du fichier
              if (cmdExecute && !cmdName) {
                cmdName = file.replace('.js', '').toLowerCase();
              }
              
              // Si on a les deux, charger la commande
        ...
        // Si aucune commande n'a été trouvée mais que le module exporte quelque chose
...
// =================== AUTO-REDÉMARRAGE MÉMOIRE ===================
function checkMemoryAndRestart() {
  const memUsage = process.memoryUsage();
  const heapUsedMB = Math.round(memUsage.heapUsed / 1024 / 1024);
  
  if (heapUsedMB > config.MEMORY_LIMIT) {
    logger.error(`🚨 Mémoire critique: ${heapUsedMB}MB > ${config.MEMORY_LIMIT}MB - Redémarrage forcé`);
    process.exit(1);
  }
}
...
async function startBot() {
  try {
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
      ...
      if (!cmd) {
        // Debug: afficher les commandes disponibles si la commande n'est pas trouvée
      ...
      try { 
        // Vérifier que cmd.execute est bien une fonction
