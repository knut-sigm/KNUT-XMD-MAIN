...
export async function execute(sock, msg, args, from) {
  try {
    ...
    if (!arg || !["on", "off"].includes(arg)) {
      const current = getGroupProtections(from).respons ? "activé" : "désactivé";
      await sock.sendMessage(from, { 
        text: `> Knut XMD: Respons\n\nÉtat : ${current}\n\nUtilisation : \`!respons on\` ou \`!respons off\``
      }, { quoted: msg });
      return;
    }

    const newState = arg === "on";
    setGroupProtection(from, "respons", newState);

    await sock.sendMessage(from, { 
      text: `> Knut XMD: Respons ${newState ? "activé" : "désactivé"} dans ce groupe.`
    }, { quoted: msg });
